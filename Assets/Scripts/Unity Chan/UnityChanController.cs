...
public class UnityChanController : MonoBehaviour, ISaveable
{
    ...
    // TCP stuff (moved to TCPServer.cs script)
    ...
    // Message: I seperated the TCP part from the controller script for custom port support.
    // Please open the TCPServer.cs script instead.
    // However, the parse string part is kept in below since it's related to the model.
    // Sorry for the inconvenience if you are coming from the existing YT tutorial.
